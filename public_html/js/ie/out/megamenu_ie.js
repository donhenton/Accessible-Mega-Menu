...
var MegaMenu = function () {
  function MegaMenu(menu, defs) {
    ...
    this.subPanels = [];
    Array.from(this.menu.querySelectorAll("." + this.defaults.topNavItemClass)).forEach(function (topNav) {
  ...
  _createClass(MegaMenu, [{
    key: "skipKeyDown",
    value: function skipKeyDown(ev) {
      var tgt = document.querySelector(this.skipTargetSelector);
    ...
    key: "skipClick",
    value: function skipClick(ev) {
      var tgt = document.querySelector(this.skipTargetSelector);
      tgt.focus();
    }
  }, {
    key: "skipFocus",
    value: function skipFocus(ev) {
    ...
    key: "menuMouseOver",
    value: function menuMouseOver(ev) {
      if (this.selectedMenuIdx) {
        this.resetPanels();
      }
    }
    ...
    value: function resetPanels() {
      this.inMenu = false;
      this.selectedMenuIdx = null;
      this.subPanels.forEach(function (p) {
    ...
    key: "advanceMenu",
    value: function advanceMenu(amt) {
      var newIdx = (this.selectedMenuIdx + amt) % this.subPanels.length;
    ...
    key: "updateMenuPanels",
    value: function updateMenuPanels(newSelectedId) {
      var _this = this;

      this.subPanels.forEach(function (p, idx) {
        //  console.log(`idx ${idx} `)
        if (p.panelUUID === newSelectedId) {
          _this.selectedMenuIdx = idx;
          p.displayMenu(true);
        } else {
          p.displayMenu(false);
        }
...
var MegaSubPanel = function () {
  function MegaSubPanel(topNav, menuParent) {
    _classCallCheck(this, MegaSubPanel);
    ...
    var linkBlock = this.panelLink.getBoundingClientRect();
  ...
  _createClass(MegaSubPanel, [{
    key: "setAsLast",
    value: function setAsLast(b) {
      var me = this;
      this.isLast = b;
      ...
      this.endTab.addEventListener('focus', function (ev) {
        me.menuParent.resetPanels();
        var start = document.querySelector(me.menuParent.defaults.contentStart);
        if (start) {
          start.focus();
        }
      });
    ...
    key: "panelKeyDown",
    value: function panelKeyDown(ev) {
    ...
    key: "displayMenu",
    value: function displayMenu(show) {
      // let status = 'none';
      if (show) {
        status = 'block';
    ...
    value: function linkFocus(ev) {
      ...
      if (this.menuParent.defaults.allowMainLinkNav) {
        ...
        window.setTimeout(function () {
          console.log("focus " + _this2.panelUUID);
          if (me.menuParent.isClick === true) {
            return;
          }
          me.isSelected = true;
          me.menuParent.updateMenuPanels(_this2.panelUUID);
        }, 50);
      }
      me.isSelected = true;
